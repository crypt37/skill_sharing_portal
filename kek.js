...
const verifyCallback = (username, password, done) => {
    ...
    connection.query('SELECT * FROM students WHERE sid = ? ', [username], function(error, results, fields) {
        if (error)
            return done(error);
...
app.post('/register', userExists, (req, res, next) => {
    ...
    connection.query('Insert into students(sid,hash,salt,gender,std_name,std_image,std_about,age) values(?,?,?,?,?,?,?,?) ',
